<?php
	...
	$redeem_canvas_user[] = get_user_meta($user_id, '_canvas_user_id', true);
	...
	if (!empty($redeem_user_count)) {
		$usernames = $wpdb->get_results("SELECT user_login, ID 
			FROM {$wpdb->prefix}users 
			WHERE ID IN (" . implode(',', $redeem_user_count) . ")");

	}


	    $user_type = get_user_meta($user_id,'user_type',true);
		if (is_user_logged_in() && $user_type == 'EPP') {
				...
				?>
					<div class="search_loc_cover i">
			
						<div class="main_dash_candidate">
				
							<p class="candidate_head">Candidate</p>
							<div class="candidate-dash-back-contain">
								<a href="<?php echo $url.'/portal/' ;?>"  class="candidate_dashboard_back">Back</a>
							</div>
					 
							
						</div>
						<div class ="text-center1">
							<div class="form-candidate-searchs">
								...
								<div class="custom-input-search search_input_loc" style="display:none;">		
									...
									<select name="" id="custom_candidate_data">
										<option class="candidate_option"value="">All</option>
										<?php 
											...
											// 		$first_name_value = get_user_meta($ID,'first_name',true);
											// 		if(empty($first_name_value)){
											// 			$first_name_value = $course_search_val;
											// 		}
													?>
													<div>
														<option class="candidate_option" value="<?php //echo $ID;?>"> <?php //echo $first_name_value;?></option>
													</div>
													<?php
											// 	}
											// }
								...
								<div class="search_icon_add">
									<button id="search-candidate-button-click" name="Search"  class="Buttoncreate">
					...
					<div class='username_container'>
						<table  class="table table-striped table-condensed"  id="tblData_candidate">
							<thead>
								<tr>
									<th>Name</th>
									<th class="tab2">Avg Diagnostic</th>
									<th class="tab2">Avg Practice</th>
									<th class="tab2 module">Module</th>
						...
						foreach ($usernames as $usernames_array_value) {
							...
							if ($candidate_search_value != '') {
								...
								if (isset($courses_by_user[$ID])) {
									...
									// echo'<pre>';
                                    // print_r($final_to_process_for_user);
									// echo'</pre>';
					...
					<div class="cus_candidate_coursename" id="cus_candi_course"></div>
		...
		<script>
			jQuery(document).ready(function(){
				jQuery('#tblData_candidate').DataTable({
				...
				jQuery(document).on('click','.cus_candidate_option',function(){
					jQuery('.username_container').css('display','none');
					jQuery('.search_loc_cover').css('display','none');
					jQuery('#cus_candi_course').css({
							'display': 'block', 
										'border': '1px solid #043753', 
										'padding': '20px', 
										'border-radius': '5px', 
										'margin-top': '20px' 
									});
					jQuery('#cus_candi_course').html('<div style="width:100%;height:200px;text-align:center;"><img  class="loading-gif" src="/wp-content/uploads/2025/03/loading-gif.gif"></div>');
					var ID = this.getAttribute('data-ID');
					var candidate_name = jQuery(this).data('candidate_name');
					jQuery.ajax({
						type: 'post',
						url:my_ajax_object.admin_ajax_url,
						data:{
							'action':'candidate_course_name',
							'ID':ID,
						...
						success:function(data){
							...
							jQuery('#cus_candi_course').html(data)
						},
				...
				jQuery(document).on('click','.coursename_back',function(){
					jQuery('.cus_candidate_coursename').css('display','none');
					jQuery('.username_container').css('display','block');
					jQuery('.search_loc_cover ').css('display','block');
					
				});
				jQuery(document).on('click','.reports_back',function(){
					jQuery('.test').css('display','none');
					jQuery('.cus_candidate_coursename').css('display','block');
					
				});	
				jQuery(document).on('click','#search-candidate-button-click',function(){
					var startDate = jQuery('#startdate').val();
					var endDate = jQuery('#enddate').val();
					if (startDate === '' && endDate=== '') {
					return false; 
					}
					var candidate_name =jQuery('.candidate_option:selected').val();
					jQuery('.username_container').html('<div style="width:100%;height:200px;text-align:center;"><img  class="loading-gif" src="/wp-content/uploads/2025/03/loading-gif.gif"></div>');
						jQuery.ajax({
							type: 'post',
							url: my_ajax_object.admin_ajax_url,
							data: {
							...
							success:function(data){
								...
								jQuery('.username_container').html(data);
							},
							error: function(request, errorThrown){
								console.log(errorThrown);
							}
				...
				jQuery(document).on('click','.c_course',function(){
					jQuery('.cus_candidate_coursename').css('display','none');
					var canvas_user_ids = this.getAttribute('data-user-id');
					var product_id = this.getAttribute('data-product-id');
					var course_id = this.getAttribute('data-course_id');
					var product_name =  jQuery(this).data('product_name');
					var candidate_name = jQuery(this).data('candidate_name');
					var userid_new = jQuery(this).data('userid_new');

					jQuery('.test').css('display','block');
					jQuery('.test').html('<div style="width:100%;height:200px;text-align:center;"><img  class="loading-gif" src="/wp-content/uploads/2025/03/loading-gif.gif"></div>');
					jQuery.ajax({
						type: 'post',
						url:my_ajax_object.admin_ajax_url,
						data:{
						'action':'candidate_inner_ajax',
						'canvas_user_ids':canvas_user_ids,
